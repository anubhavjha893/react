...
const Card = ({handleClick,key,item,handleRemove,index}) => {
  const {name,email,image} = item
  return (
    <div className='w-60 h-full bg-zinc-800 rounded-md p-5'>
 ...
 <h1 className='opacity-60 text-xs font-semibold text-white leading-[1.2vh] ml-1'>{email}</h1>
       ...
       <button onClick={()=>handleRemove(index)} className='bg-red-600 hover:bg-zinc-600 text-white capitalize text-sm font-bold px-3 p-1 rounded-md ml-[60px] mt-3'>remove</button>
    </div>
  )
}
